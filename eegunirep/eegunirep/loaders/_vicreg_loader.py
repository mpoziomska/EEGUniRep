...
from eegunirep.transforms import OnlineTransforms
...
class VicRegLoader(BaseLoader):
    ...
    def __init__(self, csv_path, config, device, loglevel, profiler=False, dtype=torch.float32, io_size=(1048576 * (2**6)), io_path=None, io_mode='pickle', label_col='label'):
        ...
        self.data_dir = io_path
        self.transforms = OnlineTransforms(device=self.device, dtype=self.dtype, config=config, profiler=profiler, loglevel=loglevel)
